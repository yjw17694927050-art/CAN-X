...
/**
 * The Runtime boundary's three failure types are defined once, in `./runtime-http`, and
 * re-exported here because this is where the DBC workspace and the DBC tests import them
 * from. A caller that branches on `RuntimeDbcApiError` must see *the* class, not a second
 ...
 *
 * This module builds the write request; the shared transport owns what an unreachable
 * Runtime means. The rejection's own text is never forwarded: it can carry a URL, an
 * address or a browser message about the network, none of which tells a user anything
 * they can act on.
 */
async function post(input: ImportDbcAssetInput): Promise<Response> {
  return runtimeFetch(`${RUNTIME_URL}${DBC_ASSETS_PATH}`, {
    body: JSON.stringify({
 ...
 * Runtime defined. What an unreachable Runtime means is the shared transport's
 * business, not this module's.
 */
async function get(url: string): Promise<Response> {
  return runtimeFetch(url, { method: "GET" });
}

"""A genuine V0.2-01 project must open, migrate, and keep working under V0.2-02.

The V0.2-01 runtime wrote a metadata-only database stamped ``user_version = 1``.
This is the compatibility gate: such a project is upgraded in place, keeps its
identity, and gains the ability to hold data sessions.
"""
...
def test_opening_a_v1_project_migrates_it_in_place(tmp_path: Path) -> None:
    ...
    assert stored_schema_version(root) == DATABASE_SCHEMA_VERSION == 2
    assert {"project_metadata", "data_sessions", "data_segments"} <= stored_tables(root)
...
def test_the_migration_is_idempotent_across_repeated_opens(tmp_path: Path) -> None:
    ...
    assert stored_schema_version(root) == 2

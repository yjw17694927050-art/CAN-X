...
## 9. AUDIT-FIX-03-R2 — live DBC decode request fragmentation

Round 2 addresses the one P1 the independent verdict left open. Nothing else in §3 was touched:
DataSession, Query, reconnect, capture and Dockview are as they were, and no integration test in
this round gave a reason to revisit them.

### 9.1 Root cause

`DecoderCache` (§3.3) removed the *repeated static work* — parse and compile — but not the
*request count*. The partitioner still cut a viewport along the capture batch's axis:

```text
seq1 can0 → DBC-A
seq2 can1 → DBC-B      same asset × contiguous sequence × MAX_BATCH_FRAMES
seq3 can0 → DBC-A  ⇒   1 frame / request
seq4 can1 → DBC-B      with one request in flight: N frames ≈ N HTTP requests
```

Measured before the fix, on this machine, 400 alternating frames over two different DBCs:
**400 requests · 1.0 frames/request · 1.267 s · 316 frames/s** — against a virtual capture that
produces 1000 frames/s. The cache was working; the request shape was the ceiling.

### 9.2 Why `FrameBatch` must not be relaxed

A `FrameBatch` is a *slice of one stream's numbering*: its sequences are consecutive because the
stream numbers everything it carries, and `first_sequence` / `last_sequence` describe a range. That
is a property of **capture**, and the recorder, the persistence layer and the sequence-continuity
invariant (§3.1) all rest on it. A DBC decode is a **frame-local, stateless transformation**: a
decoder asked about sequence 1, 3 and 5 answers exactly what it would answer for each of them alone,
and never looks at sequence 2. Widening the capture container to carry "frames with holes" would
have traded a real invariant for a decoding convenience.

### 9.3 The new abstraction and the API strategy

```text
runtime/canx/dbc/decode_set.py
  DecodeFrameSet   one stream_id · 1..1000 canonical frames · caller order preserved
                   sequences strictly increasing — NOT required to be contiguous
  DecodedFrameSet  one outcome per submitted frame, in submission order; strictly increasing;
                   no first/last sequence, because a set has no range

POST /dbc/assets/{asset_id}/decode-frames   (new)   the ordered work set
POST /dbc/assets/{asset_id}/decode-batch    (kept)  the contiguous FrameBatch, unchanged
```

Both endpoints reach the same `DbcDecoder` through the same `ProjectDbcService.load_decoder`, so the
file read, the size + SHA-256 proof and the content-keyed decoder cache are shared rather than
reimplemented. The new surface adds a request *shape*, not a decode path. `/decode-batch` keeps its
contract: `1, 3` is still refused, and a single test asserts both surfaces against **one** payload
(`test_a_non_contiguous_payload_diverges_between_the_two_surfaces`).

Strictly increasing (rather than merely "unique") was chosen so that a caller's own order is the
contract: a repeat would mean two outcomes for one frame, and a backwards step would mean the caller
did not send what it says it sent. `DecodeFrameSet.MAX_FRAMES` is where the 1000 lives; the HTTP
request model reads that constant instead of restating it.

### 9.4 Desktop partition: before / after

```text
before   same asset × contiguous GLOBAL sequence × MAX_BATCH_FRAMES
after    per asset, in viewport order, cut only at MAX_BATCH_FRAMES
```

```text
A1 B2 A3 B4 A5 B6        before: 6 requests     after: 2   (A[1,3,5], B[2,4,6])
400 alternating / 2      before: 400 requests   after: 2
2500 frames, one asset   before: 3 requests     after: 3   (unchanged — the bound, not the gaps)
unbound channel present  contributes nothing, and fragments nothing
```

`decode-partition.test.ts` keeps every scenario it had — interleaving, gaps, unbound channels, the
frame bound, input order, the invalid bound — with the expectations rewritten to the new contract,
and gains the headline cases above. `decode-coordinator.test.ts` and
`live-decode-flows.integration.test.tsx` observe the same change through the real coordinator: the
alternating viewport is two requests, and every sequence is still decoded exactly once.

### 9.5 Measurement — this machine, this run

```text
== request fragmentation (one viewport, real handlers) ==
scene                plan           assets  frames    req frames/req elapsed_s     frames/s  parse compile  read
1-one-channel        BEFORE 1/run        1     400      1      400.0     0.019       21,400      1       1     1
1-one-channel        AFTER 1/asset       1     400      1      400.0     0.019       21,064      1       1     1
2-two-ch-shared      BEFORE 1/run        1     400      1      400.0     0.019       20,848      1       1     1
2-two-ch-shared      AFTER 1/asset       1     400      1      400.0     0.019       21,275      1       1     1
3-two-ch-two-dbc     BEFORE 1/run        2     400    400        1.0     1.267          316      2       2   400
3-two-ch-two-dbc     AFTER 1/asset       2     400      2      200.0     0.023       17,402      2       2     2
4-large-viewport     BEFORE 1/run        2    4000   4000        1.0    13.474          297      2       2  4000
4-large-viewport     AFTER 1/asset       2    4000      4     1000.0     0.562        7,114      2       2     4
5-repeated-viewport  BEFORE 1/run        2    4000   4000        1.0    12.609          317      2       2  4000
5-repeated-viewport  AFTER 1/asset       2    4000     20      200.0     0.214       18,706      2       2    20

== request fragmentation over real HTTP (FastAPI/ASGI, in process) ==
scene                plan            frames    req elapsed_s   ms/req     frames/s
3-two-ch-two-dbc     BEFORE 1/run       400    400     3.062     7.66          131
3-two-ch-two-dbc     AFTER 1/asset      400      2     0.048    24.18        8,271
```

The structural acceptance case — 400 alternating frames over two assets — is **400 requests → 2**,
and the request count no longer grows with the frame count for this viewport. The AFTER throughput is
measured at 17,402 frames/s on the handlers and 8,271 frames/s through the real ASGI application,
both far above the 1000 frames/s the virtual capture produces: the fragmentation ceiling is gone
rather than merely reduced. The large-project scenes now import **two genuinely different** documents
(`synthetic_a` at 0x200; `synthetic_b` at 0x600 with an `Alt` message prefix, asserted unequal).
Two assets holding one SHA-256 would have been one document read twice, and the content-keyed cache
would have answered the second asset from the first asset's entry.

The existing cache BEFORE/AFTER table is retained in the same script and the same run.

### 9.6 Cache integrity — unchanged

`load_decoder` still reads and verifies the project-owned bytes on every request, before any cache
lookup; a tampered asset still fails closed with `dbc.asset_integrity_failed` (409), and the new
endpoint has its own test for it (`test_a_decode_work_set_is_integrity_checked_like_any_other_decode`).
No global "current DBC" and no Runtime-side channel binding were introduced: `channelId → assetId`
remains the Workspace Session's.

### 9.7 Changed files

```text
runtime/canx/dbc/decode_set.py                    NEW — DecodeFrameSet + DecodedFrameSet
runtime/canx/dbc/decode.py                        decode_frames, one shared outcome loop
runtime/canx/api/dbc.py                           POST …/decode-frames; shared DecodeOutcomeResponse

apps/desktop/src/workspace/decode-partition.ts    groups by asset; a gap no longer splits a run
apps/desktop/src/workspace/decode-coordinator.ts  submits work sets; still one request in flight
apps/desktop/src/runtime/dbc-decode-client.ts     decodeDbcFrameSet (ordered, non-repeating)
apps/desktop/src/components/workspace/DockWorkspace.tsx   wires the new call

tests/unit/dbc/test_dbc_decode_set.py             NEW — the container contract
tests/unit/dbc/test_dbc_decoder_frames.py         NEW — decode_frames across gaps
tests/unit/dbc/bench_dbc_hot_path.py              fragmentation BEFORE/AFTER + HTTP measurement
tests/unit/api/test_dbc_api.py                    the decode-frames HTTP contract
tests/integration/test_dbc_api_integration.py     interleaved assets over ASGI
tests/integration/test_packaged_runtime_smoke.py  decode-frames round trip from the frozen .exe
apps/desktop/src/**/*.test.ts(x)                  partition / coordinator / client / flow tests
```

§7's "no new endpoint" statement describes round 1; it is no longer true of the branch.

### 9.8 Gates on this tree

```text
python -m pytest -q                            2902 passed · 1 skipped
                                               (the skip is an environment limit: this shell cannot
                                                create a directory link without privileges)
python -m ruff check runtime tests tools       clean
python -m mypy runtime tools/agent tools/ci    100 files, clean
pnpm lint · pnpm typecheck · pnpm build        clean
pnpm test                                      33 files · 533 tests passed
packaged runtime smoke                         7 passed, against a sidecar rebuilt from this tree
```

### 9.9 Developer status

```text
AUDIT-FIX-03-R2:  READY FOR INDEPENDENT ACCEPTANCE
